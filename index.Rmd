...
### Espacio métrico
De acuerdo a [@Barnsley] un espacio métrico es un par $(X,d)$,
donde $X$ es un conjunto no vacío de elementos a los que llamaremos
puntos y $d$ es una función de $X\times X$ en ${R}$ que
cumple los siguientes axiomas:

* $d(a,b)\geq0$.
* $d(a,b)=0\Longleftrightarrow a=b$.
* $d(a,b)=d(b,a)$ (axioma de simetría).
* $d(a,b)\leq d(a,c)+d(c,b)$ (desigualdad triangular)Para todo $a,b$ 
y $c$ en $X$.  


La función $d$ se llama distancia o métrica en $X.$

Un ejemplo clásico de espacio métrico es el conjunto de los números
complejos y la distancia usual dada por el módulo de un número complejo.
De acuerdo a [@articlechurchil] se tiene que 
$$d(z,w)=\sqrt{\cc{x_{1}-x_{2}}^{2}+\cc{y_{1}-y_{2}}^{2}},$$
donde los números complejos son $z=x_{1}+iy_{1}$ y $w=x_{2}+iy_{2}$.
La teoría de los espacios métricos surge con el objetivo de generalizar
los espacios euclidianos, permitiendo extender ideas a otros tipos
de conjuntos. El siguiente ejemplo muestra como verificar los axiomas
sobre un supuesto espacio métrico.
...
El campo de variabilidad de la sucesión $\set{x_{n}}$ es 
\[
\set{y\in X:\exists n\in\mathbb{N}:x_{n}=y},
\]
es decir, el conjunto de valores que toma la sucesión. Una sucesión
se dice acotada si su campo de variabilidad es un conjunto acotado. 
...
$d(p,x_{n})<\epsilon$. En ese caso, decimos que la sucesión converge
hacia $p,$ o que $p$ es el limite de $\set{x_{n}},$ y escribimos
$\lim_{n\rightarrow\infty}x_{n}=p.$ Observemos que decir que $\lim_{n\rightarrow\infty}x_{n}=p$
es equivalente a decir que $\lim_{n\rightarrow\infty}d(x_{n},p)=0.$
...
Sea $(X,d)$ un espacio métrico completo, $x\in X$, $A$ y $B\in\mathfrak{H}(X)$.
Se define 
\[
d(x,B)=\min\set{d(x,y):y\in B}.
\]
$d(x,B)$ es la distancia desde el punto $x$ al conjunto $B$. 
...
### Transformaciones holomorfas y analíticas  
...
transformaciones visto en diferentes sistemas de coordenadas
